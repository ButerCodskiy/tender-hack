...
class FactCheckingGuard:
    ...
    def extract_footnotes(self, text: str) -> list[int]:
        """Извлекает 1-based порядковые номера сносок ([^1] или [1])."""
        return [int(m) for m in self.FOOTNOTE_REGEX.findall(text)]

    def extract_numeric_facts(self, text: str) -> set[str]:
        """Извлекает нормализованные числовые значения, проценты, суммы и даты.

        Предварительно вырезает маркеры сносок ([^N] и [N]), исключая ложное
        срабатывание на индекс сноски как на ненайденный числовой факт.
        """
        # 1. Исключаем сами сноски, чтобы не проверять цифру индекса
        cleaned = self.FOOTNOTE_REGEX.sub("", text)
        # 2. Склеиваем пробелы между цифрами (например, "500 000" -> "500000")
        normalized = re.sub(r"(?<=\d)\s+(?=\d)", "", cleaned)
        # 3. Нормализуем слеши в датах (например, "15/10/2024" -> "15.10.2024")
        normalized = re.sub(r"(?<=\d)/(?=\d)", ".", normalized)
        # 4. Извлекаем последовательности цифр с точками или запятыми
